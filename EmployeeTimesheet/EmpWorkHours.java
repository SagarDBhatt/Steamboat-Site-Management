...
public class EmpWorkHours {
	...
	private void initialize() {
		...
		frmRevolutionSystemsEmployee.setIconImage(Toolkit.getDefaultToolkit().getImage(EmpWorkHours.class
		...
		panelEmplTS.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Employee Timesheet",
				TitledBorder.LEFT, TitledBorder.TOP, null, new Color(0, 0, 0)));
		panelEmplTS.setBounds(12, 13, 388, 130);
		frmRevolutionSystemsEmployee.getContentPane().add(panelEmplTS);
		panelEmplTS.setLayout(new FormLayout(
		...
		comboBoxEmplName.setModel(new DefaultComboBoxModel(new String[] { "Danir Hernandez", "Heidy Alcantar",
				"Jaime Sevilla Hernandez", "Maynor Galeano Martinez", "Ruth Alcantar", "Wilmer Sevilla" }));
...
//Action Listener for Submit button!
		...
		btnSubmit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					ConnectionToSQL.ConnectionSql();
					if(comboBoxWorkHours.getSelectedItem().toString() != null && dateChooser != null) {
						EmployeeInteraction.insertEmployeeData(comboBoxEmplName.getSelectedItem().toString(),
								Double.parseDouble(comboBoxWorkHours.getSelectedItem().toString()), dateChooser);
					}
					else
						JOptionPane.showMessageDialog(null, "Please enter valid Workhours & Date!", "Error!!",
								JOptionPane.ERROR_MESSAGE);
				} catch (NumberFormatException | NullPointerException e) {
					JOptionPane.showMessageDialog(null, "Please enter valid Workhours & Date!", "Error!!",
							JOptionPane.ERROR_MESSAGE);
				} catch (ClassNotFoundException e) {
					JOptionPane.showMessageDialog(null, "Class not found in Employee class", "Error!!",
							JOptionPane.ERROR_MESSAGE);
				} catch (SQLException e) {
					JOptionPane.showMessageDialog(null, "Unable to Connect SQL Database", "Error!!",
							JOptionPane.ERROR_MESSAGE);
				}
...
//End of ActionListener for Submit button
